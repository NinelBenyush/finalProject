...
function Messages() {
    ...
    useEffect(() => {
        async function fetchUploadedFiles() {
            try {
                const response = await axios.get('http://localhost:5000/latest-upload');
            ...
            >
                ...
                {uploadResponse && (
                    <li role="article" className="relative pl-6">
                        ...
                        <div className="flex flex-col flex-1 gap-0">
                            <h4 className="text-sm font-medium text-slate-700">
                                {uploadResponse.another_m}
                            </h4>
                            <p className="text-xs text-slate-500">{uploadResponse.upload_time}</p>
                        </div>
                    </li>
                )}
                <li role="article" className="relative pl-6">
